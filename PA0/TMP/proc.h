...
struct	pentry	{
	...
	int	pwaitret;		/* check if process is in start - stop block	*/
	/*	Addition of below 4 lines for PA0	*/
	int	count_sys_calls[27];	/* store count to each system calls 	*/
	int	avg_execution_times[27];	/* store average execution times to each  system calls	*/
	int start_time[27];		/*	start times of each system calls	*/
	int track;				/*	used to identify if proc is being monitored	*/
...
//struct  pentry  *custom_proc = &proctab[currpid];
//int count_sys_calls[27];
