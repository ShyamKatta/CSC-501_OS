...
int resched()
{
	...
	case LINUXSCHED :
		...
		if(epoch > 0 && process_to_schedule()>0){	// continue in old epoch. only current process goodness, quantum change
			...
			process_counter+=1;
			proctab[currpid].quantum = preempt; //(ctr1000-proctab[currpid].last_start_counter);
		...
		//else{
			...
			for(i=0; (i<NPROC); i++){
				if((proctab[i].pstate==PRFREE))
					continue;
				//kprintf("\n curr process is %d, %s, %d ", i, proctab[i].pname, proctab[i].goodness);
				//kprintf("i value %d\n",i); 
				...
				proctab[i].last_start_counter= 0;
	...
	proctab[currpid].last_start_counter = process_counter;
...
int getProcess(){
	int high_goodness=0;
	int high_proc=0;
	int last_start = process_counter;
	int temp = q[rdytail].qprev;
	while( temp!=q[rdyhead].qnext){
		if(proctab[temp].goodness>=high_goodness){
			//kprintf("\tstart times %d, %d, %d, %d\t",high_proc, temp, proctab[high_proc].last_start_counter, proctab[temp].last_start_counter);
			if(proctab[temp].goodness==high_goodness){ //to check for round robin
				if(proctab[temp].last_start_counter<last_start){
					//high_goodness = proctab[temp].goodness;
					last_start= proctab[temp].last_start_counter;
					high_proc = temp;
				}
			}
			else{
