...
struct	pentry	{
	...
	int	pwaitret;
	/***** added for linux scheduler   ****/
	int epoch_flag;		/* to see if the process is included in epoch*/
	int quantum; 		/* remaining time for which process can execute*/
	int goodness;		/* based on which schuduler picks up process*/
	long last_start_counter;	/*	last time when the process was kicked in in this epoch - set to zero at begining of epoch*/
	int	epoch_start_pprio;	/*	priority captured at start of epoch	*/
};
...
extern 	int epoch;			/* Used for linux like scheduler	*/
