					...
					/* array in the code)		*/
{
	...
	//	 logic to save lock which process requested to lockarr[lock descriptor].
	unsigned long *templock;
	templock = (unsigned long *)(&args);
	lockarr[*(++templock)].requested_lock[pid]=1;	// logic to handle delete
	//	logic to update -1 to priority inheretence 
